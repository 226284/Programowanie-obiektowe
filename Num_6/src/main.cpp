...
#include "BlokCalkowania.hh"
#include "BlokWzmocnienia.hh"
#include "BlokPrzesuniecia.hh"
...
int main (int argc, char **argv)
{
  ...
  while ((c = getopt(argc,argv,"i:o:GCD:SW:P:")) != -1)
    {
      switch (c)
	{
	...
	case 'C':
	  {
	    ListaB.DodajBlokSygnalowy (new BlokCalkowania());
	...
	case 'W':
	  {
	    BlokWzmocnienia *tmp = new BlokWzmocnienia();
	    tmp->set_wzmocnienie(atof(optarg));
	    ListaB.DodajBlokSygnalowy (tmp);
	...
	case 'P':
	  {
	    BlokPrzesuniecia *tmp = new BlokPrzesuniecia();
	    tmp->set_przesuniecie(atof(optarg));
	    ListaB.DodajBlokSygnalowy (tmp);
